...
mod_select_map_server <- function(id, r) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$clear_all_polygons, {
      ...
      showNotification("All polygons cleared.", type = "warning")
    })
    ...
    observeEvent(input$reset_extent, {
      ...
      showNotification("Reset to original extent and resolution", type = "message")
    })
    ...
    return(list(
      ...
      selected_polygon_geometry = reactive({
        if (!is.null(selected_polygon()) && selected_polygon() %in% names(polygon_list())) {
